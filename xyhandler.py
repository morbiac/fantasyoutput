...
class xYHandler(object):
  ...
  def __init__(self, authf):
    self.authf = authf
    self.authd = {'consumer_secret': config.get('Yahoo', 'Consumer_Secret'),
                  'consumer_key': config.get('Yahoo', 'Consumer_Key')}
    self.yahoo = OAuth1Service(consumer_key=config.get('Yahoo', 'Consumer_Key'),
                               consumer_secret=config.get('Yahoo', 'Consumer_Secret'),
                               name='yahoo',
  ...
  def reg_user(self):
    ...
    time.sleep(1)
    self.authd['oauth_verifier'] = input('Please enter your PIN:')
